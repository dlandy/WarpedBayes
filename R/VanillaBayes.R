...
#' Calculate the log of the odds for a set of proportions.
#'
#' Utility function to convert proportions into log odds (truncating proportions <.005 and >.995)
#' @param proportions A vector of proportions, between 0 and 1
#' @export
#' @examples
#' logOdds()
logOdds <- function(proportions) {
  proportions[proportions==1] <- 0.995
  proportions[proportions==0] <- 1-0.995
  d <- log(proportions/(1-proportions))
  d
}
...
#' Calculate the proportions for a vector of logs of the odds
#'
#' Utility function to convert log odds into proportions. This function does not truncate, so it does not 
#' truly invert logOdds
#' @param proportions A vector of log odds, between negative and positive infinity
#' @export
#' @examples
#' logOdds()
...
#' @export
#' @examples
#' HollandsDyreGonzalezWuAsymmericForwardModel(c(1,2))
HollandsDyreGonzalezWuAsymmericForwardModel <- function(stimuli, beta=1, delta=1, scaling=1, leftScaling=scaling, rightScaling=scaling, center=0.5){
...
#' Create Predicted data patterns from a vanilla gonzalez & Wu model
#'
#' This model calculates, given a set of parameters, the predicted results of an 'asymmetric' HD GW model. This 
#' model uses a central (but maybe off-center) reference point, and applies the G&W model from that central point ot each end
#' Each end may be separately scaled.
...
#' @export
#' @examples
GonzalezWuForward <- function(stimuli,gamma, delta, scaling=1){
  center <- 0.5
  x <- center-(center-stimuli)*scaling
  perception <- (delta*stimuli^gamma) / 
    ((delta*stimuli^gamma + (1-stimuli)^gamma))
  result <- center-(center-perception)/scaling
}



#' Create Predicted data patterns from a vanilla Stevens
#'
#' This model calculates, given a set of parameters, the predicted results of an 'asymmetric' HD GW model. This 
#' model uses a central (but maybe off-center) reference point, and applies the G&W model from that central point ot each end
#' Each end may be separately scaled.
...
#' @export
#' @examples
stevens <- function(stimuli, alpha=1, beta=1){
  alpha*stimuli^beta
}
